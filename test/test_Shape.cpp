...
TEST_CASE("TEST 8: Square - Comprehensive test suite") {
    // Default unit square centered at the origin, lying on the XY plane, bounded between x = -0.5 and x = 0.5, y = -0.5 and y = 0.5)
    Square square;

    SUBCASE("Intersection with ray hitting the square (Center)") {
        Ray ray{Point{0.0f, 0.0f, 2.0f}, Vec{0.0f, 0.0f, -1.0f}};
        auto hit = square.ray_intersection(ray);
    ...
    SUBCASE("Ray missing the square (Parallel)") {
        Ray ray{Point{0.0f, 0.0f, 0.0f}, Vec{0.0f, 0.0f, -1.0f}};
        auto hit = square.ray_intersection(ray);
        CHECK_FALSE(hit.has_value());
    }
    ...
    SUBCASE("Non-trivial oblique intersection and normal verification") {
        Ray ray{Point{0.5f, 0.5f, 2.0f}, Vec{0.0f, 0.0f, -1.0f}};
        auto hit = square.ray_intersection(ray);

        REQUIRE(hit.has_value());
        CHECK(hit->hit_point.is_close(Point{0.5f, 0.5f, 0.0f}));
        CHECK(hit->hit_normal.is_close(Normal{0.0f, 0.0f, 1.0f}));
    }

    SUBCASE("Boundary check: ray missing due to bounds (x > 0.5)") {
        Ray ray{Point{0.6f, 0.4f, 2.0f}, Vec{0.0f, 0.0f, -1.0f}};
        auto hit = square.ray_intersection(ray);
        CHECK_FALSE(hit.has_value());
    }

    SUBCASE("Boundary check: ray missing due to bounds (y < -0.5)") {
        Ray ray{Point{0.5f, -0.6f, 2.0f}, Vec{0.0f, 0.0f, -1.0f}};
        auto hit = square.ray_intersection(ray);
        CHECK_FALSE(hit.has_value());
    }
}